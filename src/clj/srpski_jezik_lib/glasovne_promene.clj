...
(def suglasnici
     "бвгдђжзјклљмнњпрстћфхцчџш")

(def suglasnici-bez-vjlljr
     "бгдђжзкмнњпстћфхцчџш")
...
(defn nepostojano-a
  ...
  [rec
   nastavak]
  (let [rec (cstr/lower-case
              rec)
        nastavak (cstr/lower-case
                   nastavak)]
    (if (nepostojano-a?
          rec
          nastavak)
...
(defn- jotovanje-b-transformacija
  ...
  (let [duzina-nastavka (count nastavak)
        nastavka-bez-prvog-slova (if (> duzina-nastavka
                                        1)
                                   (.substring
                                     nastavak
                                     1
...
(defn- jspz?
  ...
  (let [indeks-poslednjeg-slova (dec (count rec))
        ...
        poslednje-slovo-reci-zvucno (zvucno?
                                      poslednje-slovo-reci)
        predposlednje-slovo-reci-zvucno (zvucno?
                                          predposlednje-slovo-reci)
        poslednje-slovo-reci-bezvucno (bezvucno?
                                        poslednje-slovo-reci)
        predposlednje-slovo-reci-bezvucno (bezvucno?
                                            predposlednje-slovo-reci)]
